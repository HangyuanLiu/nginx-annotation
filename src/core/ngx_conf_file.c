...
ngx_conf_parse(ngx_conf_t *cf, ngx_str_t *filename)
{
    ...
    /*这个枚举类型表示当前解析过程处于一个什么状态*/
    enum {
        parse_file = 0,//正要开始解析一个配置文件，Nginx启动时开始配置文件解析属于这种情况
        parse_block,//正要开始解析一个复杂配置项值，如events,http
        parse_param//正要开始解析命令行参数配置项值,如nginx -g 'daemon on'
    } type;
    ...
    for ( ;; ) {
    	/*
    	 * 从配置文件中读出token,每次将从文件内读取足够多的内容以填满
    	 * 一个大小为NGX_CONF_BUFFER(4096)的缓存区,
    	 * 这个缓存区在函数ngx_conf_parse()内申请并保存引用到变量cf->conf_file->buffer内
    	 */
...
ngx_conf_set_flag_slot(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    if (*fp != NGX_CONF_UNSET) {	//检查UNSET值
        return "is duplicate";
    }

    value = cf->args->elts;		//获取参数字符串数组
    //大小写无关比较 on|off字符串,设置值为1|0
