...
typedef struct {
    ngx_uint_t  events; //调用epoll_wait方法时传入的第3个参数maxevents,而第2个参数events数组的大小也是由它决定的
...
static ngx_command_t  ngx_epoll_commands[] = {
		 ...
		 * 这个配置项表示调用一次epoll_wait时最多可以返回的事件数,它也会预分配那么多epoll_event
		 * 结构体用于存储事件
...
ngx_epoll_init(ngx_cycle_t *cycle, ngx_msec_t timer)
{
    ...
    if (ep == -1) {
    	 ...
    	 * 能够处理的最大事件个数,因为在许多Linux内核版本中,epoll是不处理这个参数的,
    	 * 所以设为cycle->connection_n/2也不要紧
    	 * */
...
ngx_epoll_add_event(ngx_event_t *ev, ngx_int_t event, ngx_uint_t flags)
{
    ...
    ngx_connection_t    *c;
    struct epoll_event   ee;
    //每个事件的data成员都存放这其对应的ngx_connnection_t连接
    c = ev->data;
    //下面会根据event参数确定当前事件是读事件还是写事件,这会决定events是加上EPOLLIN标志位还是EPLLLOUT标志位
    ...
    ngx_log_debug3(NGX_LOG_DEBUG_EVENT, ev->log, 0,
                   ...
                   c->fd, op, ee.events);
    //调用epoll_ctl方法向epoll中添加事件或者修改事件
...
ngx_epoll_process_events(ngx_cycle_t *cycle, ngx_msec_t timer, ngx_uint_t flags)
{
    ...
    for (i = 0; i < events; i++) {
    	/*
    	 * 对照着上面提到的ngx_epoll_add_event方法,
    	 * 可以看到ptr成员就是ngx_connection_t连接的地址,
    	 * 但最后1位有特殊含义,需要把它屏蔽掉
    	 * */
        ...
        if ((revents & EPOLLOUT) && wev->active) {
            ...
            if (flags & NGX_POST_EVENTS) {
                ...
                wev->handler(wev);//立即调用写事件的回调方法来处理这个事件
            }
        }
    }
