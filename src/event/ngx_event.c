...
//负载均衡机制实现的关键阈值,这个阈值与连接池中连接的使用情况密切相关
...
static ngx_command_t  ngx_event_core_commands[] = {
	//连接池大小，也就是每个worker进程中支持的tcp最大连接数
...
/*
 * 循环调用ngx_process_events_and_timers方法就是在处理所有的事件,这正是事件驱动机制的核心
 * 主要工作:
...
ngx_process_events_and_timers(ngx_cycle_t *cycle)
{
    ...
    //ngx_use_accept_mutex = 1时代表开启负载均衡
    if (ngx_use_accept_mutex) {
    	//通过检测变量ngx_accept_disabled的值是否大于0来判断当前进程是否已经过载
        if (ngx_accept_disabled > 0) {
            ...
            if (ngx_accept_mutex_held) {
            	/*
            	 * 如果当前拥有锁,则给flags变量打个NGX_POST_EVENTS标记.
            	 * 这表示所有发生的事件都将延后处理.这是任何架构设计都必须遵守的一个约定,
            	 * 即持锁者必须尽量缩短自身持锁的时间
            	 * */
                ...
                {
                	//没有抢到锁，需要让ngx_prcess_change方法在没有新事件的时候，至少等待ngx_accept_mutex_delay毫秒再去试图抢锁
                    timer = ngx_accept_mutex_delay;
                }
            }
        }
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_EVENT, cycle->log, 0,
                   "timer delta: %M", delta);
    //如果ngx_posted_accept队列不为空，那么调用函数执行队列中需要建立新连接的事件
