...
//第一个http模块，负责管理http模块的很多基础工作，比如server解析
...
ngx_http_core_run_phases(ngx_http_request_t *r)
{
    ...
    cmcf = ngx_http_get_module_main_conf(r, ngx_http_core_module);
    //ngx_http_request_t结构体中的phase_handler成员将决定执行到哪一个阶段
    //以及下一个阶段应当执行哪个http模块实现的内容
    ph = cmcf->phase_engine.handlers;	//获取引擎数组首地址

    while (ph[r->phase_handler].checker) {
    	//执行checker,间接调用handler
...
ngx_http_core_generic_phase(ngx_http_request_t *r, ngx_http_phase_handler_t *ph)
{
    ...
    rc = ph->handler(r);
    //如果handler方法返回NGX_OK,之后将进入下一个阶段处理,而不会理会当前阶段中是否还有其它处理方法
    if (rc == NGX_OK) {
        r->phase_handler = ph->next;
        return NGX_AGAIN;
    }
    //如果handler方法返回NGX_DECLINED,那么将进入下一个处理方法,这个
    //处理方法既可能属于当前阶段,也可能属于下一个阶段
    if (rc == NGX_DECLINED) {
        r->phase_handler++;
        return NGX_AGAIN;
    }
    ...
    //如果handler返回上诉类似的返回码,则调用这个函数结束请求
    ngx_http_finalize_request(r, rc);

    return NGX_OK;
}

//ngx_http_core_rewrite_phase是NGX_HTTP_SERVER_REWRITE_PHASE阶段(第二阶段)的checker方法
ngx_int_t
ngx_http_core_rewrite_phase(ngx_http_request_t *r, ngx_http_phase_handler_t *ph)
{
    ...
    if (rc == NGX_DECLINED) {
        r->phase_handler++;	//表示将要执行下一个回调方法
        return NGX_AGAIN;	//返回NGX_AGAIN,HTTP框架不会把进程控制权交给epoll事件框架,而是继续立刻执行请求的下一个回调
    }

    /*
     * handler方法返回NGX_DONE,则意味着刚才的handler方法无法在这一次调度中处理完
     * 这一个阶段,它需要多次的调度才能完成.
     * 此时返回NGX_OK,它会使得http框架立刻把控制权交还给epoll事件模块,不再处理当前请求
     * 唯有这个请求上的事件再次被触发时才会继续执行
     * */
    if (rc == NGX_DONE) {
        return NGX_OK;
    }
...
//这一阶段用于控制用户发起的请求是否合法,如检测客服端的ip地址是否允许访问
ngx_int_t
ngx_http_core_access_phase(ngx_http_request_t *r, ngx_http_phase_handler_t *ph)
{
    ...
    if (r != r->main) {
    	//如果当前请求只是一个派生出的子请求的话,是不需要执行NGX_HTTP_ACCESS_PHASE阶段
    	//的处理方法的,那么直接将phase_handler设为下一个阶段(实际上是NGX_HTTP_POST_ACCESS_PHASE)的处理方法的序号
        r->phase_handler = ph->next;
        return NGX_AGAIN;
    }
...
//NGX_HTTP_CONTENT_PHASE阶段用于真正处理请求的内容
ngx_int_t
    ...
    ngx_http_phase_handler_t *ph)
{
    ...
    if (r->content_handler) {	//检查是否有location内容处理函数
    	 ...
    	 * 也就是告诉http框架再有可写事件时就调用ngx_http_request_empty_hander直接把控制权
    	 * 交给事件模块
    	 * */
        r->write_event_handler = ngx_http_request_empty_handler;
        ngx_http_finalize_request(r, r->content_handler(r)/* 调用内容处理函数　*/);
        return NGX_OK;
    }
    ...
    rc = ph->handler(r);	//没有location特定的处理函数，执行此模块的handler
    ...
    ph++;	//模块返回NGX_DECLINED，需要继续执行本阶段的下一个handler

    if (ph->checker) {	//下一个模块有checker
