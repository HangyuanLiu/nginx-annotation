...
//upstream框架代码,初始化请求,调用用户自定义回调函数 create_request
static void
...
ngx_http_upstream_handler(ngx_event_t *ev)
{
    ...
    //这个连接并不是Nginx与客户端的连接,而是Nginx与上游服务器的连接
    c = ev->data;
    //由连接的data成员取得ngx_http_request_t结构体
    r = c->data;
    //由请求的upstream成员取得表示upstream机制的Ngx_http_upstream_t结构体
    u = r->upstream;
    //ngx_http_request_t结构体中的这个connection连接是客户端与Nginx的连接
    c = r->connection;
    ...
    if (ev->write) {
    	//当Nginx与上游服务器间tcp连接的可读事件被触发时,upstream的方法会被调用
    ...
    ngx_http_upstream_t *u)
{
    ...
    if (c->write->timedout) {
    	//将超时错误传递给ngx_http_upstream_next方法,根据错误重连策略,重新发起连接执行请求
    ...
    if (u->header_sent) {
    	/* header_sent为1时一定是已经解析完全部的上游响应包头,并且开始向
    	 * 下游发送http包头了.此时,是不应该继续向上游发送请求的,所以把
    	 * write_event_handler设为任何工作都没有做的ngx_http_upstream_dummy_handler方法
    	 * */
        u->write_event_handler = ngx_http_upstream_dummy_handler;
        //将写事件添加到epoll中
    ...
    //本函数(ngx_http_upstream_send_request_handler)方法更多的时候是在
    //检测请求的状态,而实际复制发送请求的方法是ngx_http_upstream_send_request
    ngx_http_upstream_send_request(r, u, 1);
}
...
//upstream框架在调用create_request()得到请求数据后就会向上游发送请求
//然后在ngx_http_upstream_process_header里处理接收到的数据
static void
ngx_http_upstream_process_header(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
    ...
    if (!r->subrequest_in_memory) {	//检查标识位,直接转发响应体
    ...
    if (u->input_filter == NULL) {	//是否有input_filter回调函数,没有则使用默认函数处理
        u->input_filter_init = ngx_http_upstream_non_buffered_filter_init;
        u->input_filter = ngx_http_upstream_non_buffered_filter;
        u->input_filter_ctx = r;
    }

    if (u->input_filter_init(u->input_filter_ctx) == NGX_ERROR) {	//过滤处理初始化
    ...
    if (n) {
        ...
        if (u->input_filter(u->input_filter_ctx, n) == NGX_ERROR) {	//过滤处理响应体数据
            ngx_http_upstream_finalize_request(r, u, NGX_ERROR);
            return;
        }
    }
