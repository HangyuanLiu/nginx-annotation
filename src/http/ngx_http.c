...
ngx_http_block(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    char                        *rv;
    ngx_uint_t                   mi, m, s;
    ngx_conf_t                   pcf;	//临时保存ngx_conf_t
    ngx_http_module_t           *module;
    ngx_http_conf_ctx_t         *ctx;	//http块的环境数据
    ...
    ctx = ngx_pcalloc(cf->pool, sizeof(ngx_http_conf_ctx_t));	//创建环境数据
    if (ctx == NULL) {
        return NGX_CONF_ERROR;
    }
    /* conf是传入函数的参数，在调用http框架前(ngx_conf_file.c中)，
     * 已经将conf赋值为指向cycle->conf_ctx数组的合适位置的指针
    */
    *(ngx_http_conf_ctx_t **) conf = ctx;
    ...
    for (m = 0; ngx_modules[m]; m++) {
        if (ngx_modules[m]->type != NGX_HTTP_MODULE) {	//检查http模块
            continue;
        }
        ...
        if (module->create_main_conf) {	//创建main配置项
        ...
        if (module->create_srv_conf) {	//创建server配置项
        ...
        if (module->create_loc_conf) {	//创建location配置项
    ...
    rv = ngx_conf_parse(cf, NULL);	//使用新的ctx解析块内指令
